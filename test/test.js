...
test("tests if response.js will return an array including the searched word and the 4 that come after it", function(t) {
   t.deepEquals(response.arrayMaker("A"), ['A', 'a', 'aa', 'aal', 'aalii'], "There's an array baby!!");
   t.end();
});

test("tests if randomDefintion() will return a random definiton from an array of words ", function(t) {
  var result = typeof response.randomWord(["cat", "dog", "bird"]);
  var expected = "string";
...
test("tests if we can grab the API key", function(t) {
   t.deepEquals(response.printApi().length, 49, "we hid the api key, such magic so magician ");
   t.end();
});
test("does the getAPIobject function return the word that we searched", function(t) {
   ...
   response.getDefinition(process.env.DB_API, "book", function(word) {
      result = word;
   });
